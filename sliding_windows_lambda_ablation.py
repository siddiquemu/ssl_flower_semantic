...
class sliding_windows(object):
    ...
    def sliding_window(self):
        ...
        for y in range(0, self.img.shape[0] - self.step_size[1], self.step_size[1]):
            for x in range(0, self.img.shape[1] - self.step_size[0], self.step_size[0]):
                ...
                yield (x, y, x + self.window_size[0], y + self.window_size[1],
                       self.img[y:y + self.window_size[1], x:x + self.window_size[0]])
    ...
    def overlay_masks(self, img_bin=None, im=None, out_dir=None, imgname=None,
                      dpi=200, box_alpha=0.2, gt=None, evaluation=None, nums=None, show_class=False,
                      show_gt_contours=False):
    ...
    def remap_preds(self, mask, img_org, angle):
        ...
        mask_rerot = [imutils.rotate_bound(mask, -angle) for mask in
                      mask_rot]  # mask_image scaled to original image size
        ...
        if len(img_props_ind) > 0:
            ...
            if len(proposals) > 0:
                ...
                if jitter_boxes:
                    ...
                    aug_proposals = aug_box_jitter(aug_proposals, times=jitter_size,
                                                   frac=jitter_factor)  # size: (len(fr_boxes),10,1,4)
    ...
    def rgr_refine(self, pred, img):
        ...
        ## RGR parameters
        # thresholds
        tau0 = 0.5  # original CNN threshold 0.5
        tauF = 0.8  # high confidence foreground 0.8
...
if __name__ == '__main__':
    ...
    apply_regression = 0
    apply_test_aug = 0
    rotation_sample_factor = 1  # #rotation_sample_factor angles from 5 ranges
    vis = 0
    apply_rgr = 0
    plot_PR = 1
    iteration = 1
    ...
    for j, lambda_s in enumerate([0.9]):#np.arange(0.1, 1, step=0.1):
        # experiment on all dataset
        if j>0:
            init_params['data_already_registered'] = True
        for i, data_type in enumerate([dataset]):  # ['AppleB', 'AppleA', 'Peach', 'Pear']
            ...
            if model_type == 'SSL_RGR':
                ...
                init_params['model_path'] = f'{model_path}/{data_type}/lambda_ablation/lambda_{lambda_s}/model_0019999.pth'
            else:
            ...
            for img_path in test_frames.values[1:]:
                ...
                for (x1, y1, x2, y2, window) in SWs.sliding_window():
                    ...
                    if apply_regression:
                        ...
                        if instance_model:
                            pred_mask_ins = SWs.instances_to_semantic(window, pred_results['instances'],
                                                                      box_score_thr=init_params['pred_score'])
                            ...
                            pred_mask_ins = SWs.apply_regression(pred_mask.clone(), window, seg_score_thr=0.5,
                                                                 box_score_thr=init_params['pred_score'], area_thr=100,
                                                                 jitter_size=30, jitter_factor=0.1, jitter_boxes=True)
                    else:
                        pred_mask_ins = None
                    ...
                    assert window.shape[:2] == pred_mask.cpu().numpy().shape[
                                               1::], f'found {window.shape[:2]}=={pred_mask.cpu().numpy().shape[1::]}'
                    window_id += 1
                ...
                if not plot_PR:
                    ...
                    d = [[f'iter{iteration}', data_type, "iou: %.3f" % iou, "F1: %.3f" % F1, "Rcll: %.3f" % Rcll,
                          "Prcn: %.3f" % Prcn]]
            ...
            if not apply_rgr and plot_PR:
                ...
                F1_score = []
                IOU_score = []
                for seg_score_thr in np.arange(0.4, 0.7, 0.02):
                    eval_mask = evaluate_mask(gpu_id=init_params['gpu_id'])
                    iou, F1, Rcll, Prcn, nums = eval_mask.overall_metrics(pred_ms, gt_ms, score_thr=seg_score_thr,
                                                                          area_thr=100)
                    #print(f'IoU:{iou}, F1:{F1}, Rcll:{Rcll}, Prcn:{Prcn}')
                    ...
                    #count stats
                    # fl_counter = flower_counter(score_thr=seg_score_thr)
                    # fl_stats = fl_counter.get_dataset_flower_stats(gt_ms, pred_ms)
                    # print(fl_stats)
                print(f'Iter: {iteration}, lambda_s: {lambda_s} max iou {np.max(IOU_score)} at F1 max {np.max(F1_score)}')
                iou_all.append(np.max(IOU_score))
                ...
                #PR_Data.to_csv(f'PR_{data_type}_iter{iteration}_{model_type}.csv')
                plt.close()
            else:
                eval_mask = evaluate_mask(gpu_id=init_params['gpu_id'])
