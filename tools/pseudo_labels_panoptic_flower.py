...
import cv2
import time
#from tools.MI_MS_clasp2_annotations import delete_all
import torch
import imutils
...
from init_pseudo_labels_dir import get_all_dirs, delete_all
...
class Pseudo_Labels(panoptic_fpn_flower):
    ...
    def mask_to_box(self, mask, fr_num, img_id=None):
        ...
        for i in img_props_ind[::-1]:
            ...
            if area>100:
                ...
                #ann_id = int('{}{}'.format(self.p_id, self.init_params['ann_id_index'][0]))
                #color = self.all_rgb[ann_id]
                #print(f'color {color} for ann: {ann_id}')
                ...
                #since the rgb masks are not used in learning, only update binary mask for filtered flower as background
                # already initialized as background mask
                #self.img_mask_semantic[label_mask==i] = 1 #1:back:flower
                pass
                #print(f'instance area<200')
    ...
    def get_augmented_proposals(self, fr_num):
        ...
        if self.init_params['database']=='flower':
            ...
            self.angleSet = self.get_random_angles(ranges=self.ranges, factor=4)
        ...
        for angle in random.sample(self.angleSet, len(self.angleSet)):
            ...
            if fr_num not in self.semi_frames:
                ...
                if not self.init_params['apply_rgr']:
                    pred_mask = pred_mask_C[self.flower_class_id]
    ...
    def pseudo_labels_main(self):
        ...
        for fr_num in random.sample(self.img_subset, len(self.img_subset)):
            ...
            if fr_num not in [0.482560000,0.475980000, 0.492030000]: 
...
if __name__ == '__main__':
    ...
    storage = args.working_dir
    num_gpus=args.number_gpus

    models_dir = 'Panoptic_Models'
    ...
    init_params['local_storage'] = '/media/siddique/6TB2/test_aug_gt_pan'
    if not os.path.exists(init_params['local_storage']):
        os.makedirs(init_params['local_storage'])
    ...
    if args.database in ['flower']:
        ...
        # use 0.5++
        if num_gpus==2:
            init_params['cuda_list'] = [0,1]
        if num_gpus==3:
           init_params['cuda_list'] = [1, 2]
        ...
        # currently unused: useful to discard nosy samples from the augmented proposals
        ...
        #train/test: Peach: 0.1, 0.16, 0.6, 0.5, 0.58, 0.52; 
        # AppleA: 0.32, 0.56, 0.5, 0.48
        # AppleB: 0.4, 0.52, 0.42
        # Pear: 0.32, 22, 0.48
        #init_params['remap_score_thr'] = init_params['sem_thr'][args.data_set][args.ssl_iter]
        #full test set: Pear: 0.2, AppleA: 0.4, Peach: 0.48:iter6, AppleB: 0.36:iter4, 0.4:iter1, 0.52:iter2
