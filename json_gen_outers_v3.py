...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug  4 12:56:57 2021
...
def pan_ann_gen(outers,segIDs,imgID,bboxes,areas, instance_certainty=-1, semantic_certainty=-1):
    segment_info=[]
...
def inst_ann_gen(outers,segIDs,imgID,bboxes,areas, ann_type='box_mask', instance_certainty=-1):
    inst_ann=[]
...
def define_categories_dictionary(mode):
    if mode=='panoptic':
        categories=[
            {
            'id': 1,
...
#%%function for converting bmask to a single channel image where background
#pixels have value 2 and flowere pixesls have value 1

def smask_gen(bmask):
    assert bmask.dtype==np.uint8
    if len(bmask.shape)==3:
        bmask=bmask[:,:,0]
    smask=np.zeros(bmask.shape)
    smask[bmask>1]=0    #assign flower pixels
    smask[bmask==0]=1   #assign background pixels
    smask=smask.astype(np.uint8)
    smask=Image.fromarray(smask).convert('L')
    return smask
...
if __name__ == '__main__':
    train_test='train'
    ann_type='box'
    data_for_model = 'SoftTeacher'

    NAS = '/media/NAS/Walden/trainTestSplit/'

    base_dir = NAS+train_test+'/dataFormattedProperly/splitImages4x3/'

    bmask_dir=base_dir+'bMasksSplit/'
    flower_dir=base_dir+'flowersSplit/'
    rgbmask_dir=base_dir+'rgbMasksSplit/'
    smask_dir=base_dir+'semMasksSplit/'
    ...
    for i in range(len(imgIDs)):
        ...
        smask=smask_gen(img)
        smask.save(smask_dir+mask_filenames[i])
        ...
        inst_ann=inst_ann_gen(outers,segIDs,imgID, bboxes, areas, ann_type=ann_type)
    ...
    cats_pan=define_categories_dictionary('panoptic')
    cats_inst=define_categories_dictionary('instance')

    panopticjson={}
    panopticjson['images']=imgs
    panopticjson['annotations']=pan_anns
    panopticjson['categories']=cats_pan

    s=json.dumps(panopticjson)
    with open(base_dir+'AppleA_panoptic_split4x3_'+train_test+'.json','w') as f:
        f.write(s)

    instancejson={}
    instancejson['info']=[]
    instancejson['images']=imgs
    instancejson['licenses']=[]
    instancejson['annotations']=inst_anns
    instancejson['categories']=cats_inst

    s=json.dumps(instancejson)
    # with open(base_dir+'AppleA_instance_split4x3_box_'+train_test+'.json','w') as f:
    #     f.write(s)
    with open(f'/media/NAS/Walden/SoftTeacher/data/flower/annotations/instances_{train_test}_2021.json','w') as f:
        f.write(s)
