...
def plot_uncertainty_map(pred, save_path, img_path):
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    # getting data of the histogram
    flower_pixels = pred[pred>0.1]
    count, bins_count = np.histogram(flower_pixels.flatten(), bins=10)
    ...
    plt.legend()
    file_path = os.path.basename(img_path).split('.')[0]+'.png'
    plt.savefig(f'{save_path}/{file_path}', dpi=200)
    ...
    for i, data_type in enumerate(['Peach']):#['AppleB', 'AppleA', 'Peach', 'Pear']
        ...
        for img_path in folder:
            ...
            if init_params['data'] in ['AppleA','AppleA_train']:
                ...
                step_size = [img.shape[1] // 16, img.shape[0] // 16]  # [step_w, step_h]: 8,6 or 16,12 # 16 16, 8,8
                window_size = [img.shape[1] // 8, img.shape[0] // 8]  # [w_W, w_H] #4,3 or 8,6
            ...
            if plot_uncertainty:
                save_path = '{}/uncertainty_map'.format(init_params['overlay_out_dir'])
                plot_uncertainty_map(final_pred.copy(), save_path, img_path)
        ...
        #     F1_score = []
        #     IOU_score = []
        #     for seg_score_thr in np.arange(0.04,0.6, 0.02):
