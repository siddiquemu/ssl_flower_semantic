...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='semi')
...
if args.database=='flower' and args.model_type == 'SSL':
    cfg = load_labeled_data(args, cfg)

    storage = f'{args.working_dir}/tracking_wo_bnw/data/flower/test_aug_gt_pan/lambda_exp/{args.data_set}/iter{args.ssl_iter}'
...
if args.ssl_iter==0:
    ...
    cfg.MODEL.WEIGHTS = f'{model_dir}/{args.label_percent}_percent/{args.data_set}/iter{args.ssl_iter-1}/model_0024999.pth'
    if not os.path.exists(cfg.MODEL.WEIGHTS):
        cfg.MODEL.WEIGHTS = f'{model_dir}/{args.label_percent}_percent/{args.data_set}/iter{args.ssl_iter-1}/model_0019999.pth'
    if not os.path.exists(cfg.MODEL.WEIGHTS):
        cfg.MODEL.WEIGHTS = f'{model_dir}/{args.label_percent}_percent/{args.data_set}/iter{args.ssl_iter-1}/model_0014999.pth'
print(f'loaded weights: {cfg.MODEL.WEIGHTS}')
# Directory where output files are written: lambda ablation dir
cfg.OUTPUT_DIR = f'{model_dir}/{args.label_percent}_percent/{args.data_set}/lambda_ablation/lambda_{round(args.lambda_sem,2)}'
